...
async def wait_for_event(event: asyncio.Event, timeout: float = 300, description: str = "event") -> bool:
    """
...
def is_model_available(container) -> bool:
    ...
    return (container and 
            container.model_loaded and 
            not container.model_is_loading and 
            not getattr(container, 'model_is_unloading', False))
...
class ModelStateManager:
    ...
    def set_state(self, state: ModelState):
        ...
        logger.info(f"Model state changing: {self.current_state.value} -> {state.value}")
    ...
    async def wait_for_no_active_generations(self, timeout: float = 300) -> bool:
        ...
        logger.info(f"Waiting for {self.active_generations} active generations to complete")
    ...
    async def increment_active_generations(self, request_id: str, model_name: str = None, params: Dict = None):
        """
        ...
        logger.info(f"Starting generation {request_id}, active generations: {self.active_generations}")
    ...
    async def decrement_active_generations(self, request_id: str):
        ...
        if request_id in self.active_generation_info:
            ...
            logger.info(f"Finished generation {request_id} in {duration:.2f}s, active generations: {self.active_generations}")
            
            # Remove from tracking
            del self.active_generation_info[request_id]
        else:
            logger.info(f"Finished generation {request_id}, active generations: {self.active_generations}")
