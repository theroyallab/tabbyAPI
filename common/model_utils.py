...
async def handle_model_unloading_error(request_id: str, operation: str) -> Dict[str, Any]:
    """
...
async def check_model_before_operation(request_id: str, operation: str) -> Union[Dict[str, Any], None]:
    """
    ...
    if model.container is None or getattr(model.container, 'model_is_unloading', False):
        logger.warning(f"Model is being unloaded, cannot start {operation} for request {request_id}")
        return {
            ...
            if hasattr(model.container, 'increment_active_generations'):
                ...
                if hasattr(model.container, 'active_generation_info'):
                    model.container.active_generation_info[request_id] = {
                        ...
                        "model": model.container.model_dir.name if model.container.model_dir else None,
                        "params": {k: v for k, v in kwargs.items() if k != 'embeddings'}  # Exclude large objects
                    }
                
                logger.info(f"Starting generation {request_id}, active generations: {model.container.active_generations}")
    ...
    if model.container and hasattr(model.container, 'active_generations_lock'):
        async with model.container.active_generations_lock:
            if hasattr(model.container, 'decrement_active_generations'):
                ...
                if hasattr(model.container, 'active_generation_info') and request_id in model.container.active_generation_info:
                    generation_info = model.container.active_generation_info[request_id]
                    duration = asyncio.get_event_loop().time() - generation_info.get("start_time", 0)
                    logger.info(f"Finished generation {request_id} in {duration:.2f}s, active generations: {model.container.active_generations}")
                    
                    # Remove from tracking
                    del model.container.active_generation_info[request_id]
                else:
                    logger.info(f"Finished generation {request_id}, active generations: {model.container.active_generations}")
                
                # If no more active generations, set the event
                if model.container.active_generations == 0 and hasattr(model.container, 'no_active_generations_event'):
