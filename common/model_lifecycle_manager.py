...
class ModelLifecycleManager:
    ...
    def _is_model_fully_loaded_and_stable(self, model_name: str) -> bool:
        ...
        """
        return (self.state_manager.current_model_name == model_name and
                model.container and
                model.container.model_loaded and
                not model.container.model_is_loading and
                not getattr(model.container, 'model_is_unloading', False))
    ...
    def _is_expected_model_loaded(self, model_name: str) -> bool:
        ...
        """
        return (model.container and
                model.container.model_loaded and
                model.container.model_dir.name == model_name)
    ...
    def _is_model_in_transition(self) -> bool:
        ...
        """
        return (model.container.model_is_loading or
                getattr(model.container, 'model_is_unloading', False))
    ...
    async def process_switch_queue(self):
        ...
        while True:
            ...
            try:
                ...
                item = await self.switch_queue.get()  # May raise CancelledError or other exceptions.
                ...
                try:
                    ...
                    if self.state_manager.currently_loading_model is not None:
                        ...
                        try:
                            await asyncio.wait_for(self.state_manager.load_complete_event.wait(), timeout=300)
                    ...
                    if not self.state_manager.ready_for_switch_event.is_set():
                        ...
                        try:
                            await asyncio.wait_for(self.state_manager.ready_for_switch_event.wait(), timeout=60)
                        except asyncio.TimeoutError:
                            self.log_model_switch(
                    ...
                    try:
                        ...
                        asyncio.create_task(self._set_model_ready_after_delay(model_name))
                        if not future.done():
                            future.set_result(True)
                ...
                self.log_model_switch(f"Unexpected error in queue processor: {str(outer_exc)}", "ERROR")
    ...
    async def _wait_for_no_active_generations(self, model_name: str):
        ...
        if model.container:
            ...
            if not hasattr(model.container, 'state_manager') or model.container.state_manager is None:
                self.log_model_switch(f"Container does not have a state manager, cannot check active generations", "WARNING")
                return
            ...
            self.log_model_switch(f"Using state manager for waiting for active generations")
            try:
                await asyncio.wait_for(model.container.state_manager.no_active_generations_event.wait(), timeout=300)
                self.log_model_switch(f"No active generations event triggered, safe to proceed with model switch")
            except asyncio.TimeoutError:
                self.log_model_switch(f"Timed out waiting for active generations to complete", "ERROR")
    ...
    async def _perform_model_switch(self, model_name: str):
        ...
        if model.container and hasattr(model.container, 'model_is_unloading'):
            ...
            active_gens = 0
            if hasattr(model.container, 'state_manager') and model.container.state_manager:
            ...
            if self.state_manager.current_model_name == model.container.model_dir.name:
                self.log_model_switch(f"Resetting current_model_name during unload of {self.state_manager.current_model_name}")
                self.state_manager.current_model_name = None
        
        try:
            # Unload the current model
            if model.container:
                self.log_model_switch(f"Unloading existing model to switch to {model_name}")
            ...
            try:
                ...
                self.log_model_switch(f"Error loading model {model_name}: {str(e)}", "ERROR")
                # Make sure events are set to avoid deadlocks
                self.state_manager.load_complete_event.set()
                self.state_manager.ready_for_switch_event.set()
                raise
        except Exception as e:
            self.log_model_switch(f"Error during model switch operation: {str(e)}", "ERROR")
            logger.error(traceback.format_exc())
            # Make sure events are set to avoid deadlocks
            self.state_manager.load_complete_event.set()
            self.state_manager.ready_for_switch_event.set()
    ...
    async def queue_model_switch(self, model_name: str, request: Request) -> asyncio.Future:
        """
    ...
    async def load_model(self, model_name: str, request: Request):
        ...
        current_model = model.container.model_dir.name if model.container and model.container.model_loaded else "(none)"
        self.log_model_switch(
        ...
        if model.container:
            ...
            if hasattr(model.container, 'state_manager') and model.container.state_manager:
                active_gens = model.container.state_manager.active_generations
            self.log_model_switch(f"Active generations for current model: {active_gens}")
        ...
        if (
            ...
            self.log_model_switch(f"Model {model_name} is already loaded, proceeding with request")
            self.state_manager.current_model_name = model_name  # Update current model name
            return
            
        # Check if we're currently loading this exact model
        if self.state_manager.currently_loading_model == model_name:
            self.log_model_switch(f"Model {model_name} is currently being loaded, waiting for it to complete")
            try:
                # Wait for the current model load to complete
                await asyncio.wait_for(self.state_manager.load_complete_event.wait(), timeout=300)  # 5 minute timeout
                
                # After loading completes, check if the loaded model matches what we requested
                if model.container and model.container.model_dir.name == model_name and model.container.model_loaded:
                    self.log_model_switch(f"Model {model_name} has finished loading, proceeding with request")
                    self.state_manager.current_model_name = model_name
                    return
                else:
                    self.log_model_switch(
                ...
                self.log_model_switch(f"Timed out waiting for model {model_name} to load", "ERROR")
        ...
        active_gens = 0
        if model.container and hasattr(model.container, 'state_manager') and model.container.state_manager:
            active_gens = model.container.state_manager.active_generations
            
        if ((model.container and active_gens > 0) or
            (self.state_manager.currently_loading_model is not None and
             self.state_manager.currently_loading_model != model_name)):
            ...
            self.log_model_switch(f"Waiting for model switch to {model_name} to complete before processing request")
            try:
                # Set a reasonable timeout to avoid hanging indefinitely
                await asyncio.wait_for(future, timeout=300)  # 5 minute timeout
                self.log_model_switch(f"Model switch to {model_name} completed, proceeding with request")
                return
            except asyncio.TimeoutError:
                self.log_model_switch(f"Timed out waiting for model switch to {model_name}", "ERROR")
                error_message = handle_request_error(
        ...
        async with self.load_lock:
            try:
                ...
                if model.container and hasattr(model.container, 'load_lock') and model.container.load_lock.locked():
                    self.log_model_switch("Waiting for existing model operations to complete...")
                ...
                if model.container:
                    ...
                    await model.unload_model(skip_wait=False)  # Changed to False to not terminate jobs
                ...
                try:
                    ...
                    self.log_model_switch(f"Model {model_name} successfully loaded, signaling completion")
                    self.state_manager.load_complete_event.set()
                    ...
                    asyncio.create_task(self._set_model_ready_after_delay(loaded_model_name))
                ...
                if self.state_manager.currently_loading_model == model_name:
                    ...
                    if 'e' in locals() and isinstance(e, Exception):
                        ...
                        if not self.state_manager.load_complete_event.is_set():
                            self.log_model_switch(f"Model load failed or was interrupted, resetting load complete event")
                            self.state_manager.load_complete_event.set()
                        if not self.state_manager.ready_for_switch_event.is_set():
                            self.log_model_switch(f"Model load failed or was interrupted, resetting model ready for switch event")
                            self.state_manager.ready_for_switch_event.set()
    ...
    async def _set_model_ready_after_delay(self, model_name: str):
        ...
        """
        self.log_model_switch(f"Waiting {self.ready_timeout} seconds before allowing model switch")
        ...
        if not self._is_expected_model_loaded(model_name):
            self.log_model_switch(f"Not setting model ready because model {model_name} is no longer loaded")
            return
        ...
        if not hasattr(model.container, 'state_manager') or model.container.state_manager is None:
            self.log_model_switch(f"Container does not have a state manager, assuming no active generations", "WARNING")
            self.state_manager.ready_for_switch_event.set()
            return
        ...
        if model.container.state_manager.active_generations > 0:
            ...
            self.log_model_switch(f"Waiting for no_active_generations_event before setting model ready")
            try:
                ...
                if not model.container or not model.container.model_loaded or model.container.model_dir.name != model_name:
                    self.log_model_switch(f"Model changed during wait for active generations to complete")
                    return
                    
                # Also verify the model is not loading or unloading
                if model.container.model_is_loading or getattr(model.container, 'model_is_unloading', False):
                    self.log_model_switch(f"Model is now loading or unloading after waiting for active generations")
                    return
                    
                # Now safe to set the ready event
                self.log_model_switch(f"No active generations event triggered, model {model_name} is now ready for switching")
                self.state_manager.ready_for_switch_event.set()
                return
            except asyncio.TimeoutError:
                self.log_model_switch(f"Timed out waiting for active generations to complete", "WARNING")
    ...
    async def _check_model_ready_once(self, model_name: str):
        ...
        if model.container.state_manager.active_generations == 0:
            self.log_model_switch(f"Model {model_name} is now ready for switching (delayed check)")
            self.state_manager.ready_for_switch_event.set()
    ...
    async def check_model_ready_state(self) -> bool:
        ...
        if not model.container or not model.container.model_loaded:
            self.log_model_switch("Model ready check: model not loaded or container doesn't exist")
            return False
        ...
        self.log_model_switch(f"Model ready check: active_generations={model.container.state_manager.active_generations}")
