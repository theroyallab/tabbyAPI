...
| tokenizer_mode                       | String ("auto")                  | Tokenizer compatibility mode for chat formatting. Supported values are `auto`, `hf`, `mistral`; `mistral` applies Mistral-specific tool-call ID handling and falls back to default behavior on non-Mistral models.       |
