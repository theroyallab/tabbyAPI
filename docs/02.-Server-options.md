...
| log_chat_completion_requests | Bool (False)   | Writes each `/v1/chat/completions` request to `logs/debug` as JSON. Note: this includes full message history and generation parameters, with only API keys redacted. |
