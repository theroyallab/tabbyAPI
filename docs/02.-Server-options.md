...
| log_chat_completion_requests | Bool (False)   | Writes each `/v1/chat/completions` request to `logs/debug` as JSON, along with the fully templated prompt (the exact text sent to the tokenizer) as a `.txt` file with the same basename. Note: this includes full message history and generation parameters, with only API keys redacted. |
...
| seqlog_api_key            | String (None)                  | API key for Seq server, if required by server                                                                                                   |
...
| cpu_moe_offload_layers   | Int (0)                         | Number of mixture-of-expert layers to run on the CPU, with expert weights in system RAM. Only affects MoE models; set a large value such as 999 to offload all layers. Requires layer-split mode (not tensor parallel).         |
...
| template_vars_default    | Dict ({})                        | Default chat template variables, merged into every chat completion request. Values sent by the client (`template_vars`/`chat_template_kwargs`, or the top-level `reasoning_effort` field) take precedence. Example: `{reasoning_effort: high}` |
| template_vars_force      | Dict ({})                        | Like `template_vars_default`, but these override any values sent by the client.                                                                                                                                                |
| force_enable_thinking    | Bool (False)                     | DEPRECATED: alias for `template_vars_force: {enable_thinking: true}`.                                                                                                                                                          |
| reasoning                | Bool (False)                     | Enable the reasoning parser: splits the response into `reasoning_content` and `content` fields. Only enable for reasoning models.                                                                                              |
| reasoning_start_token    | String ("&lt;think&gt;")         | Start token for the reasoning parser.                                                                                                                                                                                          |
| reasoning_end_token      | String ("&lt;/think&gt;")        | End token for the reasoning parser.                                                                                                                                                                                            |
| start_in_reasoning       | String ("auto")                  | Whether generation starts inside a reasoning block. Options: auto, always, never. `auto` scans the end of the templated prompt for an unclosed reasoning start token.                                                          |
| tool_calls_in_reasoning  | Bool (True)                    | Parse tool calls that occur inside reasoning content. If False, tool call tags inside a reasoning block are treated as plain reasoning text.                                                                                   |
| reasoning_budget_tokens  | Int (None)                     | Default reasoning token budget. When a request's reasoning content exceeds it, the server forces the end of the reasoning phase by injecting `reasoning_budget_message` followed by the model's end-of-reasoning tokens. 0 ends reasoning as soon as it starts; None or negative disables. Overridable per request via `reasoning_budget_tokens` (aliases: `reasoning_budget`, `thinking_budget`, `thinking_token_budget`) or `reasoning.max_tokens`. Requires a reasoning format: reasoning tags, Harmony or Muse Glimmer. |
| reasoning_budget_message | String (None)                 | Text injected before the end-of-reasoning tokens when the reasoning budget is exhausted (default: none, matching llama.cpp — only the end-of-reasoning tokens are forced). Overridable per request via `reasoning_budget_message`. |
| tool_format              | String (None)                    | Tool calling format, e.g. `qwen3_5`. See [Tool Calling](https://github.com/theroyallab/tabbyAPI/wiki/10.-Tool-Calling) for supported formats. If left blank, tool calls from the model will not be parsed by the server.       |
| harmony                  | Bool (None)                      | Parse responses in the Harmony message format (gpt-oss models). Auto-detected from the model's special tokens by default; set to true or false to override. `tool_format: harmony` is equivalent to setting this to true. When active, supersedes the reasoning and tool format settings. |
| muse_glimmer             | Bool (None)                      | Parse responses in the Muse Glimmer message format. Auto-detected from the model's special tokens by default; set to true or false to override. `tool_format: muse_glimmer` is equivalent to setting this to true. When active, supersedes the reasoning and tool format settings. |
...
| Config Option    | Type (Default)    | Description                                                                                                                     |
| ---------------- | ----------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| draft_mode       | String ("model")  | Exllamav3 drafting mode.<br><br>Options: model, disabled, mtp, ngram                                                             |
| draft_model_dir  | String ("models") | Directory to look for draft models.<br><br>Note: Persisted across subsequent load requests                                      |
| draft_model_name | String (None)     | String: Folder name of a draft model to load. Only used by model mode.                                                          |
...
| draft_cache_mode | String ("FP16")   | Cache mode for the draft model. Specify the pair `k_bits,v_bits` where each is an integer from 2-8 (e.g. `8,8`).<br><br>The legacy values FP16, Q8, Q6 and Q4 are also accepted.                                                |
| draft_gpu_split  | List[Float] ([])  | Float array of GBs to split a draft model between GPUs.                                                                         |
| draft_num_tokens | Integer (None)    | Number of tokens to draft per iteration. Exllamav3 selects the mode-specific default when blank.                                |
| dynamic_draft    | Bool (False)      | Adjust the number of draft tokens dynamically based on observed acceptance rates. Ceiling is given by `draft_num_tokens`.       |
| ngram_match_min  | Integer (2)       | Minimum match length for exllamav3 n-gram drafting. Only used by ngram mode.                                                    |
...
| Config Option          | Type (Default) | Description                                                                                                                                                                                                        |
| ---------------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| sysmem_recurrent_cache | Int (4096)     | Max size of the recurrent cache in system memory, in MB. Only used by models with recurrent (linear or sliding attention) layers.                                                                                  |
| sysmem_kv_cache        | Int (0)        | Size of a second-tier K/V page cache in system memory, in MB. Complete pages evicted from the GPU cache are stored there and restored on prompt-cache hits instead of being recomputed. Not supported with tensor parallelism. Set to 0 to disable. |
| cuda_malloc_async      | Bool (True)    | Use the cudaMallocAsync allocator backend in Torch. Generally preferable, but try disabling it if you experience intermittent OoM errors.                                                                          |
